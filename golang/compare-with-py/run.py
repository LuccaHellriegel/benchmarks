...
go_results = run_command("UPPER_LIMIT=10000 /usr/bin/time -v ./add")
print("Go Results:")
for key, value in go_results.items():
    print(f"{key}: {value}")

python_results = run_command("UPPER_LIMIT=10000 /usr/bin/time -v python3 add.py")
print("\nPython Naive:")
for key in go_results:
    print(f"{key}: {(python_results[key] / go_results[key]):.1f}x")

python_results = run_command("UPPER_LIMIT=10000 /usr/bin/time -v python3 add_idio.py")
print("\nPython Idiomatic:")
for key in go_results:
    print(f"{key}: {(python_results[key] / go_results[key]):.1f}x")


python_results = run_command("UPPER_LIMIT=10000 /usr/bin/time -v python3 add_np.py")
print("\nPython Numpy:")
for key in go_results:
    print(f"{key}: {(python_results[key] / go_results[key]):.1f}x")
